...
/**
 * WebGL orb that pulses/rotates with live microphone level (RMS of the
 * frequency spectrum). With voice control off it renders the same orb idling.
 * Mic + GL context live and die together inside one effect, so toggling
 * `enableVoiceControl` fully releases the microphone (no lingering tracks).
 */
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  // Ref so a new callback identity never tears down the GL context/mic.
    ...
    const initMicrophone = async () => {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          // Raw signal (no browser processing) gives truer level readings.
          audio: {
    ...
    try {
      ...
      if (enableVoiceControl) {
        void initMicrophone().then((success) => {
          if (!disposed) micReady = success;
          else if (success) stopMicrophone();
        });
      }
