...
/**
 * CheckoutModal.tsx — the bridge to Kapruka's real multi-item checkout.
 *
 * Per the Kapruka MCP docs, `kapruka_create_order` is the ONLY way to hand a
 * whole cart to kapruka.com: it returns a click-to-pay URL (no Kapruka account
 * needed, prices locked for 60 minutes). There is no add-to-cart URL scheme,
 * so opening product pages can never carry the cart across. Creating the
 * order requires recipient name, phone and address — this modal collects just
 * those, then opens the pay link.
 */
...
interface CheckoutModalProps {
  ...
  /** When set, opens straight on the result screen (order already placed conversationally). */
  autoResult?: CheckoutResult;
  onSubmit: (details: CheckoutDetails) => Promise<CheckoutResult>;
  /** Fallback when order creation fails: open each item's Kapruka product page. */
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <div>
                <h2 className="text-base font-black tracking-tight text-foreground">Checkout on Kapruka</h2>
                <p className="text-[11px] font-medium text-muted-foreground">
                  {cart.length} item{cart.length === 1 ? "" : "s"} · Rs. {total.toLocaleString()} · secure click-to-pay link
                </p>
              </div>
              <button
                onClick={onClose}
              ...
              {phase !== "result" ? (
                <>
                  <div className="space-y-1.5">
                    <label htmlFor="checkout-recipient" className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
                      <User className="h-3 w-3" /> Deliver to (name)
                    </label>
                    <input
                  ...
                  >
                    {phase === "submitting" ? (
                      <><Loader2 className="h-4 w-4 animate-spin" /> Creating your Kapruka order...</>
                    ) : (
                      "Get my Kapruka pay link"
                    )}
                  </button>
                ...
                <div className="flex flex-col items-center gap-3 py-4 text-center">
                  {result?.status === "success" ? (
                    <>
                      <div className="grid h-14 w-14 place-items-center rounded-full bg-amber/15">
                      ...
                      {result.checkoutUrl && (
                        ...
                        >
                          Open Kapruka checkout <ExternalLink className="h-3.5 w-3.5" />
                        </a>
                      )}
                      <p className="text-[10px] text-muted-foreground">Prices locked for 60 minutes.</p>
